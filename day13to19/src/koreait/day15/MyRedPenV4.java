...
public class MyRedPenV4 {
	static ArrayList<MathProblem> addList ; //전역 변수로 선언
	static Scanner sc = new Scanner(System.in);
	
	public static void main(String[] args) {
		//틀린문제를 배열에 저장하고 파일쓰기 추가.(MyFileRWTest 클래스 사용)
		...
		while(true) {
			...
			int ans;
			//2)
			addList = new ArrayList<>(); //반복 시작할 때마다 새로운 ArrayList 객체를 만들어서 참조한다.
			...
			if(op=='0') {		//종료조건을 먼저 검사합니다.
				...
				//ArrayList를 배열로 변환시켜주는 방법
				MathProblem[] maths = new MathProblem[num-cnt];//틀린문제만큼의 사이즈
//				maths = addList.toArray(maths);
				int i=0;
				for(MathProblem prob : addList) {
					if(prob.isCorrect()==false){
						maths[i++]=prob;//배열에 틀린문제만 담긴다
					}
				}
				...
				MyFileRW.fileWrite("/Users/nuwoonu/Desktop/dev/test/yourmath.txt",maths);	
	...
	//메소드를 만들어서 수정한다. 메소드는 코드 재사용과 가독성을 위해서 필요하다
	static int start(int num, char op) { //리턴int? 맞은 개수, 인자 int num? 문자갯수 char op? 연산자
		int cnt=0;
		int ans;
		for(int i=0;i<num;i++) {
			MathProblem p1 = new MathProblem(op);
			addList.add(p1); //새로운 문제를 리스트에 담는다.
			...
			//sc.nextInt(); 메소드 실행의 엔터는 다음 sc.nextLine()메소드에서 가져간다. ->오류발생의 원인 
