...
//추상클래스 타입으로 배열을 선언하고 자식클래스 객체를 참조하는 예제.
		...
		//shapes 동일한 배열에 서로 다른 자식 객체를 참조하게 된다.
		shapes[0] = new ATriangle();
		shapes[1] = new ASquare();
		shapes[2] = new ATriangle();
		
//		ATriangle t = (ATriangle)shapes[1]; //실행 오류발생: 만들어진 객체와 다른 타입으로 캐스팅 못한다.
		ATriangle t = (ATriangle)shapes[2]; //정상 실행 : 만들어진 객체와 참조타입이 일치한다
